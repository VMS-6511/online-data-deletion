...
from utils import load_features, fista
import pandas as pd
...
def lr_hessian(w, X, y, lam, batch_size=50000):
    ...
    for i in range(num_batch):
        ...
        if H is None:
            ...
            H += X_i.t().mm(D[lower:upper].unsqueeze(1) * X_i)
    return (H + lam * X.size(0) * torch.eye(X.size(1)).float().to(device))
...
def return_loo_data(removal_indices, X_train, y_train):
    sorted_indices = sorted(removal_indices)
    X_rem = X_train[:removal_indices[0].numpy()]
    y_rem = y_train[:removal_indices[0].numpy()]
    for i, _ in enumerate(removal_indices[1:]):
        X_rem = torch.cat((X_rem, X_train[removal_indices[i-1].numpy()+1:removal_indices[i].numpy()]))
        y_rem = torch.cat((y_rem, y_train[removal_indices[i-1]+1:removal_indices[i]]))
    return X_rem, y_rem

print('Testing removal')
n = len(X_train)
H = lr_hessian(w_approx[:], X_train.float().to(device), y_train.float().to(device), args.lam)
print(H)
H_inv = H.inverse()
sum_grad_i = None
for i in range(args.num_removes):
    ...
    y_train = y_train.float().to(device)
    start = time.time()
    grad_i = loss_grad(w, X_train[removal_index].unsqueeze(0), y_train[removal_index].unsqueeze(0), args.lam)
    if sum_grad_i is None:
        sum_grad_i = grad_i
    else:
        sum_grad_i+=grad_i
    H_inv_i = (1 / (n)) * H_inv
    Delta = H_inv_i.mv(grad_i)
    w_approx += Delta
    H_params = H.mv(w)
    new_grad_H = sum_grad_i - H_params
    # Call Fista Here
    fista_out = fista(H, new_grad_H, args.lam, 300, x0=w).float().to(device) - w
    w_approx = w - fista_out
    b = args.std * torch.randn(X_train.size(1)).float().to(device)
    w_approx += (1 / (n-len(removals))) * Delta
    w_approx += b
    times[i] = time.time() - start
    print('Iteration %d: time = %.5fs' % (i+1, times[i]))
...
results_path = './%s/%s_%s_splits_%d_ratio_%.4f_std_%.4f_lam_%.0e_num_delete_%d_removal_IJ_sampling_%s_%d.csv' % (
