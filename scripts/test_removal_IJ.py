...
n = len(X_train)
H_inv = lr_hessian_inv(w_approx[:], X_train.float().to(device), y_train.float().to(device), args.lam)
...
for i in range(args.num_removes):
    ...
    print(removal_index)
    X_removed = X_train[removal_index].unsqueeze(0).float().to(device)
    y_removed = y_train[removal_index].unsqueeze(0).float().to(device)
    if removal_index == 0:
        X_train = X_train[removal_index+1:]
        y_train = y_train[removal_index+1:]
    elif removal_index == len(X_train)-1:
        X_train = X_train[:removal_index]
        y_train = y_train[:removal_index]
    else:
    ...
    y_train = y_train.float().to(device)
    start = time.time()
    grad_i = lr_grad(w_approx, X_removed, y_removed, args.lam)
    H_inv_i = (1 / (n-len(removals))) * H_inv
...
results_path = './%s/%s_%s_splits_%d_ratio_%.4f_std_%.1f_lam_%.0e_num_delete_%d_removal_IJ_sampling_%s_%d.csv' % (
